...
# ----------------------------- Create React Agent -----------------------------
    sqlite_agent = create_react_agent(name= "sqlite_agent",model=model, 
                               tools=[t for t in tools if t.name in ["get_schema_file","create_table","insert_data","select_data","upsert_data"]],
                               prompt="""You are SQLITE_AGENT, an expert assistant for managing SQLITE databases via MCP tools.
...
4. Collect all necessary information from the user query, such as:
   ...
   - table_name (for target SQLITE table)
...
Response: Call `select_table` with table_name="stocks2025""")
    ...
    while True:
        ...
        result = await sqlite_agent.ainvoke({"messages": chat_history})
