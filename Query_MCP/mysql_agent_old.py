...
# ----------------------------- Create React Agent -----------------------------
    mysql_agent = create_react_agent(name= "mysql_agent",model=model, 
                               tools=[t for t in tools if t.name in ["create_table","insert_data","select_data"]],
...
4. Collect all necessary information from the user query, such as:
   ...
   - table_name (for target MySQL table)
...
Response: Call `select_table` with table_name="stocks2025""")
    ...
    while True:
        ...
        result = await mysql_agent.ainvoke({"messages": chat_history})
