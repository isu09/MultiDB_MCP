...
postgres_agent = None  # placeholder

async def init_postgres():
    global postgres_agent
    # ---- MCP client setup ----
...
# ----------------------------- Create React Agent -----------------------------
    postgres_agent = create_react_agent(name= "postgres_agent",model=model, 
                               tools=[t for t in tools if t.name in ["get_schema_file","create_table","insert_data","select_data","upsert_data"]],
                               prompt="""You are Postgres_AGENT, an expert assistant for managing Postgres databases via MCP tools.
...
3. Always use the available MCP tools to perform database operations:
   - create_table(file_path, table_name): Create a table from a CSV, Excel, or JSON file.
...
4. Collect all necessary information from the user query, such as:
   ...
   - table_name (for target Postgres table)
...
8. If a table already exists or data already exists, inform the user politely.
...
Response: Call `select_table` with table_name="stocks2025""")
    ...
    while True:
        ...
        result = await postgres_agent.ainvoke({"messages": chat_history})
